...
class ForeignerPermanentResidenceIdProcessorTest {

    private ForeignerPermanentResidenceIdProcessor processor;
    ...
    @Test
    void parse15Success() {
        Optional<ForeignerPermanentResidenceIdInfo> infoOpt = processor.parse("KAZ110090123105");
        assertThat(infoOpt).isPresent();
        ForeignerPermanentResidenceIdInfo info = infoOpt.get();
        assertThat(info.getInternationalRegionInfo().getChineseShortName()).isEqualTo("哈萨克斯坦");
    ...
    @Test
    void parse18Success() {
        Optional<ForeignerPermanentResidenceIdInfo> infoOpt = processor.parse("911398199012310021");
        assertThat(infoOpt).isPresent();
        ForeignerPermanentResidenceIdInfo info = infoOpt.get();
        assertThat(info.getDomesticRegionInfo().getProvince()).isEqualTo("北京市");
        assertThat(info.getDomesticRegionInfo().getCity()).isNull();
